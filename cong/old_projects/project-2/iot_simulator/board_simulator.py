...
class BoardSimulator:
    ...
    def __init__(self, name, mac_address, board_type, clean_session):
        ...
        # register topic
        self.send_authenticate_topic = 'icse/newDevices'
        self.subcribe_topic = \
            'icse/' + self.mac_address + '/action'
        self.data_topic = 'icse/data'
        self.mqtt_client.username_pw_set(username=MQTT_CREDENTIALS['user_name'],
    ...
    def handle_msg(self, client, user_data, message):
        authentication_result = json.loads(message.payload)
        if authentication_result['type'] == 'subcribeNewDevice' \
                and authentication_result['status'] == 'OK':
